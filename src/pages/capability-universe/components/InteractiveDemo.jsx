...
const InteractiveDemo = ({ activeZone }) => {
  ...
  const currentDemo = demoContent?.[activeZone] || demoContent?.['creative-studio'];
    ...
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      ...
      <div className="p-6 border-b border-border bg-gradient-to-r from-accent/5 to-transparent">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold text-primary">{currentDemo?.title}</h3>
            <p className="text-text-secondary">{currentDemo?.description}</p>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 bg-accent rounded-full animate-pulse"></div>
            <span className="text-sm text-accent font-medium">Live Demo</span>
          </div>
        </div>
      </div>
      {/* Demo Tabs */}
        ...
        {currentDemo?.demos?.map((demo, index) => (
          <button
            ...
            className={`flex-1 px-4 py-3 text-sm font-medium transition-all duration-300 ${
              activeDemo === index
                ? 'text-accent border-b-2 border-accent bg-background' :'text-text-secondary hover:text-primary hover:bg-muted/50'
            }`}
          >
            {demo?.title}
          </button>
        ...
        >
          ...
          <div className="relative bg-muted rounded-xl overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end">
              <div className="p-6 text-white">
                <h4 className="font-bold text-lg">{currentDemo?.demos?.[activeDemo]?.title}</h4>
                <p className="text-white/80">{currentDemo?.demos?.[activeDemo]?.content}</p>
              </div>
            </div>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="w-16 h-16 bg-accent text-white rounded-full flex items-center justify-center shadow-2xl"
              >
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <h5 className="font-semibold text-primary">Try It Yourself</h5>
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  <span className="text-sm text-text-secondary">Customization</span>
                  <div className="flex items-center space-x-2">
            ...
            <div className="space-y-4">
              <h5 className="font-semibold text-primary">Features</h5>
              <ul className="space-y-2">
                <li className="flex items-center space-x-2">
                  <Icon name="Zap" size={16} className="text-accent" />
                  <span className="text-sm text-text-secondary">Instant preview</span>
                </li>
                <li className="flex items-center space-x-2">
                  <Icon name="Download" size={16} className="text-accent" />
                  <span className="text-sm text-text-secondary">Export results</span>
                </li>
                <li className="flex items-center space-x-2">
                ...
                <li className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-text-secondary">Version history</span>
                </li>
              </ul>
            </div>
          </div>
