...
const ThoughtLeadership = () => {
  ...
  const content = [
    ...
    {
      id: 4,
      category: 'case-studies',type: 'Case Study',
    ...
    {
      id: 5,
      category: 'trends',type: 'Analysis',title: "The Rise of Micro-Interactions: Why Small Details Drive Big Results",excerpt: `Our research into user behavior reveals that micro-interactions can increase engagement by up to 67%. Here's how to implement them strategically.\n\nWe analyzed 1,000+ websites to identify the micro-interactions that create the most impact on user experience and conversion rates.`,
      author: "David Park",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <motion.div variants={itemVariants} className="inline-flex items-center space-x-2 bg-accent/10 border border-accent/20 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-accent font-medium text-sm">Thought Leadership</span>
          </motion.div>
        ...
        >
          <div className="bg-gray-100 rounded-2xl p-2 inline-flex flex-wrap">
            {categories?.map((category) => (
              <motion.button
                key={category?.id}
                variants={itemVariants}
                onClick={() => setActiveCategory(category?.id)}
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl font-medium transition-all duration-300 m-1 ${
          ...
          {filteredContent?.map((item) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                {/* Title */}
                <h3 className="text-xl font-bold text-black mb-3 line-clamp-2 group-hover:text-accent transition-colors duration-300">
                  {item?.title}
                </h3>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-gradient-to-r from-accent to-red-600 rounded-full flex items-center justify-center">
                      <span className="text-white text-xs font-bold">
                        {item?.author?.split(' ')?.map(n => n?.[0])?.join('')}
                      </span>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-black">{item?.author}</div>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full border-gray-300 text-gray-700 hover:border-accent hover:text-accent"
                  iconName="ArrowRight"
                  iconPosition="right"
                >
                  Read Full Article
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-black rounded-2xl p-8 md:p-12 mt-16 text-center"
        >
          ...
          <div className="flex flex-col sm:flex-row max-w-md mx-auto space-y-4 sm:space-y-0 sm:space-x-4">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl border border-gray-600 bg-gray-800 text-white placeholder-gray-400 focus:outline-none focus:border-accent"
            />
            <Button
              variant="default"
